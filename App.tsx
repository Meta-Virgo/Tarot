        ...
        <div className="relative w-80 h-80 md:w-[450px] md:h-[450px] flex items-center justify-center pointer-events-none select-none">
            ...
            <div className="absolute inset-[25%] animate-breathing">
                <svg viewBox="0 0 200 200" className="w-full h-full text-indigo-600 drop-shadow-[0_0_15px_rgba(99,102,241,0.6)] overflow-visible">
                    <polygon points="100,10 125,80 195,80 140,125 160,190 100,150 40,190 60,125 5,80 75,80" fill="none" stroke="currentColor" strokeWidth="1.2" />
                    <polygon points="100,10 125,80 195,80 140,125 160,190 100,150 40,190 60,125 5,80 75,80" fill="currentColor" fillOpacity="0.08" stroke="none" transform="scale(0.6) translate(66 66)" />
                </svg>
            </div>
...
const RevealMagicEffect = () => (
    <div className="absolute inset-[-40%] z-0 pointer-events-none animate-magic-reveal flex items-center justify-center">
        ...
        <svg viewBox="0 0 200 200" className="w-full h-full text-indigo-400 drop-shadow-[0_0_8px_rgba(165,180,252,0.6)]">
            ...
            {/* Fine rotating rings */}
            <circle cx="100" cy="100" r="90" fill="none" stroke="currentColor" strokeWidth="0.5" opacity="0.6" />
            <circle cx="100" cy="100" r="85" fill="none" stroke="currentColor" strokeWidth="1" strokeDasharray="1 4" />
            
            {/* Geometric star (Hexagram) */}
            <g opacity="0.7">
                <polygon points="100,10 178,155 22,155" fill="none" stroke="url(#magicGrad)" strokeWidth="0.8" />
                <polygon points="100,190 22,45 178,45" fill="none" stroke="url(#magicGrad)" strokeWidth="0.8" />
            </g>
...
export default function TarotApp() {
    ...
    const toggleVoiceInput = () => {
        ...
        if (!SpeechRecognition) { alert("您的浏览器不支持语音功能"); return; }
        ...
        recognition.onresult = (e: any) => { setUserQuestion(e.results[0][0].transcript); setIsListening(false); };
        recognition.onspeechend = () => { recognition.stop(); setIsListening(false); };
        recognition.onerror = () => setIsListening(false);
    };

    const handleActionClick = async () => {
        if (aiReading) { setShowAiModal(true); return; }
        ...
        try {
            ...
            if (audioData) { setAudioUrl(pcmToWav(audioData)); }
        } catch (e) {
            console.error(e);
    ...
    const renderCardInLayout = (index: number) => {
        ...
        const cardClass = isDiamond ? 'w-[70px] h-[110px] md:w-24 md:h-40' : 'w-[80px] h-[120px] md:w-32 md:h-52';
        ...
        return (
            <div key={index} style={{ gridArea }} className={`flex flex-col items-center group relative ${!isDiamond ? 'mx-auto' : ''}`}>
                <p 
                    className={`text-[9px] md:text-[10px] font-serif uppercase tracking-[0.2em] mb-2 md:mb-4 opacity-0 animate-fade-in-up font-medium transition-all duration-500 ${isActive ? 'text-indigo-600 font-bold -translate-y-8 scale-105' : 'text-slate-400'}`} 
                ...
                <div 
                    ...
                    `}
                >
                    {/* --- Reveal Magic Effect (Transient Burst) --- */}
            ...
            <main className="flex-1 w-full max-w-6xl p-4 md:p-6 z-10 flex flex-col items-center justify-center min-h-[100vh] md:min-h-[600px]">
                    ...
                    <div className="flex flex-col items-center animate-fade-in w-full max-w-lg mt-10 md:mt-0 gap-8">
                        ...
                        <div className="w-full relative group transition-all duration-300 focus-within:scale-[1.01] px-4">
                            ...
                            <div className="relative bg-white border border-white rounded-full px-4 py-3 md:px-6 md:py-4 shadow-[0_8px_30px_-6px_rgba(0,0,0,0.05)] flex items-center gap-3 md:gap-4 transition-all duration-300 focus-within:shadow-[0_12px_40px_-6px_rgba(99,102,241,0.15)] focus-within:border-indigo-50">
                                <input type="text" value={userQuestion} onChange={(e) => setUserQuestion(e.target.value)} placeholder="Ask the cards..." className="flex-1 bg-transparent border-none outline-none focus:outline-none focus:ring-0 text-sm md:text-base text-slate-700 placeholder:text-slate-400 font-serif tracking-wide" />
                                <button onClick={toggleVoiceInput} className={`p-2 rounded-full transition-colors duration-300 ${isListening ? 'text-rose-500 bg-rose-50' : 'text-slate-300 hover:text-indigo-500 hover:bg-indigo-50'}`}>
                    ...
                    <div className="flex flex-col items-center animate-fade-in w-full h-full justify-center mt-10 md:mt-0">
                        ...
                        <div className="flex flex-wrap justify-center items-center h-56 md:h-64 w-full max-w-5xl relative">
                            {deck.slice(0, 20).map((c, i) => {
                                ...
                                return (
                                    <div key={c.uuid} onClick={() => handleCardPick(i)} className="absolute w-14 h-24 md:w-20 md:h-32 bg-white rounded-lg shadow-md cursor-pointer transition-all duration-300 hover:-translate-y-8 hover:z-50 hover:shadow-2xl hover:shadow-indigo-100 origin-bottom active:-translate-y-4" style={{ transform: `translateX(${offset * xMultiplier}px) translateY(${translateY}px) rotate(${rotation}deg)`, zIndex: 20 - Math.abs(offset) }}>
                                        <CardBack />
                                    </div>
                                )
                            })}
                ...
                {gameState === 'reading' && (
                    <div className="w-full flex flex-col lg:flex-row gap-6 md:gap-12 animate-fade-in items-center lg:items-start justify-center h-full pt-16 md:pt-10 pb-6 md:pb-20">
                        ...
                        <div className={`w-full lg:w-[400px] bg-white/95 backdrop-blur-xl border border-white rounded-t-3xl lg:rounded-3xl p-6 md:p-8 shadow-[0_-10px_40px_-12px_rgba(0,0,0,0.1)] flex flex-col transition-all duration-500 relative z-30 ${showAiModal ? 'fixed bottom-0 left-0 right-0 h-[80vh] lg:h-[600px] lg:static' : 'h-auto min-h-[300px] md:min-h-[500px]'}`}>
                            ...
                            {!showAiModal ? (
                                <div className="flex-1 flex flex-col">
                                    ...
                                    <button onClick={handleActionClick} disabled={!revealedCards.every(Boolean) || (isAiLoading && !aiReading)} className={`w-full py-3 md:py-4 rounded-xl flex items-center justify-center gap-3 mt-6 md:mt-auto transition-all duration-500 group border active:scale-95 ${revealedCards.every(Boolean) ? 'bg-slate-800 text-white hover:bg-indigo-600 hover:shadow-lg hover:shadow-indigo-200 border-transparent' : 'bg-slate-50 text-slate-300 cursor-not-allowed border-slate-100'}`}>
                                        {isAiLoading ? ( <Loader2 className="animate-spin" size={16}/> ) : aiReading ? ( <Eye size={16} className="group-hover:scale-110 transition-transform"/> ) : ( <Sparkles size={16} className="group-hover:rotate-12 transition-transform"/> )}
